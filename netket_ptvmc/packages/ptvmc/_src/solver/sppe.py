...
from ptvmc._src.solver.product_expansion import AbstractSPE, ProductExpansionState
...
class SPPE(AbstractSPE):
    ...
    def finish_step(
        ...
        """
        c = self._c[-1]
        Z, _ = split_generator(generator)

        vstate = y_t
        vstate.variables = vstate.model.apply_zz(vstate.variables, Z, scale=c * dt)
